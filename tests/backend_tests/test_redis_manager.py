...
#
# @patch("database.crud.update_session")
# @patch("database.crud.update_project")
# @patch("database.crud.get_project_users")
# @patch("database.crud.get_user_by_id")
# def test_delete_auth_session_project(
#         mock_get_user, mock_get_project_users, mock_update_project, mock_update_session, redis_manager
# ):
#     db_session = MagicMock(spec=Session)
#
#     mock_get_user.return_value = MagicMock(preference='{"store_context": true}')
#     mock_get_project_users.return_value = [MagicMock(user_id=uuid.uuid4())]
#
#     # Use a valid UUID string here instead of "auth123"
#     valid_token = str(uuid.uuid4())
#
#     # Now call delete with a valid UUID string
#     redis_manager.delete("auth_token", valid_token, db_session)
# @patch("backend.redis_manager.logging")
# def test_listen_for_expired_keys(logging_mock, redis_manager):
#     # Patch pubsub to simulate expired keys
#     mock_pubsub = MagicMock()
#     redis_manager._RedisManager__redis_client.pubsub.return_value = mock_pubsub
#
#     # Simulate listen generator yielding one pmessage for session_token_hook
#     mock_pubsub.listen.return_value = iter([
#         {"type": "pmessage", "data": "session_token_hook:token123"},
#         {"type": "pmessage", "data": "auth_token_hook:token456"},
#         {"type": "pmessage", "data": "other_key"},
#         {"type": "pmessage", "data": ""},
#     ])
#
#     # Patch delete to just pass (avoid recursion issues)
#     with patch.object(redis_manager, "delete") as mock_delete:
#         # Run listen_for_expired_keys but exit after processing (simulate by stopping iteration)
#         try:
#             redis_manager.listen_for_expired_keys(MagicMock())
#         except StopIteration:
#             pass

# Check delete called for expected keys
# assert mock_delete.call_count >= 2


# def test_cleanup(redis_manager):
#     db_session = MagicMock(spec=Session)
#
#     # Cleanup should call delete for all keys and flushdb
#     redis_manager.cleanup(db_session)
